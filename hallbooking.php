...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($booking_date < $current_date) {
        ...
        if ($hall['total_halls'] > $hall['booked_halls']) {
            // Calculate the total price (assuming price_per_day is stored in the halls table)
            $total_price = $hall['price_per_day']; // For a single day booking

            // Book the hall
            $stmt = $pdo->prepare("
                INSERT INTO hall_bookings (user_id, hall_name, event_type, booking_date, booking_time, name, mobile_number, address, total_price)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $user_id,
            ...
            $stmt = $pdo->prepare("
                INSERT INTO history (user_id, hall_name, event_type, booking_date, booking_time, name, mobile_number, address, booking_status, total_price)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'Booked', ?)
            ");
            $stmt->execute([
                $user_id,
